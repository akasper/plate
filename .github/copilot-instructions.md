...
- `.agentic/releases/` — versioned per-feature change files and migration evidence
...
- `.agentic/releases/` is the upgrade memory for PLATE process/template changes. For new Feature PRs that change PLATE behavior, author a fragment under `.agentic/releases/unreleased/<slug>.json` (not a flat version file). Fragments are aggregated at release-cut time by `scripts/cut_release.py vX.Y.Z` (or `gh plate release cut vX.Y.Z`). Use `scripts/render_release_migrations.py` to generate agent-friendly upgrade steps from the versioned directories.
- Keep per-feature fragments in `.agentic/releases/unreleased/` up to date for any PR that changes PLATE behavior; upgrade guidance should come from the fragment and rendered release-note output, not from chat history.
- PLATE uses a three-tier branch model: `epic/<name>` for feature work, `release` for integration, `main` for tagged releases. See `AGENTS.md §Branch Model and Ceremonies` for the full epic-close and release-ceremony steps. Use `gh plate release status` to inspect the current release state and `gh plate release notes --from X --to Y` to diff PLATE changes between versions.
- If `AGENTS.md`, `.agentic\process.yml`, and implementation files disagree, preserve the PLATE intent and keep them aligned.
- Labels are stable process metadata, not casual tags. Use type labels (`Bug`, `Feature`, `Epic`, `Release`, `Documentation`, `Research`, `Design`, `Question`, `Task`, `Audit`, `Migration`) and prefixed labels (`Epic:`, `area:`, `risk:`, `need:`) according to the existing taxonomy. Do not introduce `priority:` or `status:` labels; those belong in GitHub Projects fields.
...
- Every PR must carry a type label (`Bug`, `Feature`, or `Documentation`). **Critical:** The checkboxes in the PR template body do **not** apply GitHub labels — labels must be set explicitly via the CLI or GitHub API. Preferred approach: include `--label "<type>"` in `gh pr create` so the label is applied atomically at PR creation. `Feature` PRs must update per-feature change files in `.agentic/releases/`; documentation-only changes should use the `Documentation` label.
