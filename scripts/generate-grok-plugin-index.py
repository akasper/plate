#!/usr/bin/env python3
"""Generate the Grok plugin component catalog (.grok-plugin/plugin-index.json).

Vendored + lightly adapted from xAI's official plugin-marketplace
scripts/generate-plugin-index.py (https://github.com/xai-org/plugin-marketplace)
for PLATE's self-hosted "plate-marketplace" Grok Build support.

For every entry in `.grok-plugin/marketplace.json`, this script resolves the
plugin's source (local directories are scanned in place) and records which
components the plugin provides: agents, MCP servers, etc.

The output is deterministic (sorted keys, items sorted by name) so CI can
regenerate it and fail on any diff.

Usage (from repo root):
    python3 scripts/generate-grok-plugin-index.py
    python3 scripts/generate-grok-plugin-index.py --check   # for CI / release gate

See .grok-plugin/README.md (if added) and docs/ for marketplace release checklist.
"""
...
# Plate uses plain plugin.json at payload root (for cross-CLI compat).
# Grok generator also supports .grok-plugin/plugin.json inside payload root for
# explicit manifest; we keep convention scan primary so no payload change needed.
MANIFEST_PATHS = [
    Path(".grok-plugin/plugin.json"),
    Path(".claude-plugin/plugin.json"),
    Path("plugin.json"),  # support for plate's committed payload layout
]
...
def scan_mcp_servers(root: Path, manifest: dict) -> list[dict]:
    ...
    if mcp_path and mcp_path.is_file():
        ...
        if isinstance(data, dict) and isinstance(data.get("mcpServers"), dict):
            ...
            # plate's .mcp.json is top-level { "plate-core": { "command": ... } }
            # (not wrapped in "mcpServers"); support both shapes.
