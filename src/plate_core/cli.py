...
def cmd_release_finalize(args: argparse.Namespace) -> int:
    """First-class finalize for refined ceremony (part of #569 / Epic #306).
    GitHub Actions owns initial tag on merged Release PR. This surface coordinates:
    - hard-reset of release branch to tag
    - gh release create from release.json (title/body from aggregated fragments)
    - ensure next 'Next Release' issue
    - downstream .plate triggers (extensible)
    - (future) auto-close any addressed issues still open after the Release merge (complements
      the Closes block auto-generated by cut_release for one-merge-closes-everything).
    See docs/design/release-ceremony-refinement.md and AGENTS.md.
    """
    version = getattr(args, "version", None) or "vX.Y.Z"
    dry_run = getattr(args, "dry_run", False)
    releases_dir = Path(getattr(args, "releases_dir", ".agentic/releases"))
    ...
    print("Finalize steps:")
    print("  1. Tag verified/created by release.yml on merge (see validate_release_workspace).")
    print("  2. Hard-reset the appropriate release branch to the tag (legacy 'release' or release-v*).")
    print("  3. gh release create using release.json summary + body (or --generate-notes).")
    ...
    # MVP implementation: guidance + safe no-op for now (full gh api + reset in follow-up slice or when --apply added)
    # TODO (next slice): actual git reset --hard + gh release create + issue creation via GhClient.
    ...
    print("  # Portable (POSIX sh) variant:")
