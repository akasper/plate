...
- Example (one high-level goal → comprehensive fix, per #526): User: "get PR #N green". Agent: Use plate_pr_babysit + get_pr_merge_gates to inspect all gates at once, fix comprehensively (rebase, labels, threads, targeted tests), push to branch, re-inspect/repeat until only human items remain; report one-sentence summary only then.
