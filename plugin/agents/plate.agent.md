...
- Follow 'Long-running command / background task protocol' (from guidance) for background cmds in verification/babysit: immediately record task_id, proactively schedule/polling with `get_command_or_subagent_output` or `monitor` (do not wait for reminders/surface partial status at intervals), cheap fallback on kill. Consider lightweight "monitor" helper.
