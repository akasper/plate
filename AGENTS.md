...
For PLATE Q&A: **always open with** native TUI (`ask_user_question` arrow-key forms) as the first interactive move—do not start with plain-text options and only “upgrade” after the user asks (#509). Enforce full follow-through on answers (artifacts per ACs) without reminder. Offer only options whose full execution+artifacts complete in-turn before further Q&A/progress. If option promises review/babysit/address feedback, *must* fully execute via pr-babysit skill + worktree + push same branch + **resolve threads** (`plate_resolve_review_thread` / babysit `--act`; never leave feedback-resolution red after you addressed comments) (#511) before next question or progress/done. Never merge unaddressed. Named checklists: Q&A Follow-Through (#508) + Merge Gates one-pass (#510); parent doc gap #512 closed by that package. See persona/guidance + `docs/research/508-510-qa-and-pr-green-follow-through.md`. (Addresses #503, #508–#512, #517, #518, #521 and post-0.6.1 Q&A/babysit polish.)
